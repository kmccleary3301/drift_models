...
from drifting_models.drift_field import DriftFieldConfig, cfg_alpha_to_unconditional_weight
...
def main() -> None:
    ...
    if args.resume_from is not None:
        ...
        if args.resume_reset_optimizer_lr:
    ...
    if args.use_queue:
        ...
        if resume_payload is not None and isinstance(resume_payload.get("queue_state"), dict):
            queue.load_state_dict(resume_payload["queue_state"])
    ...
    if args.use_feature_loss:
        feature_extractor = _build_feature_extractor(args=args, device=device)
    ...
    if resume_payload is not None and not args.resume_reset_scheduler:
    ...
    for step in range(start_step, args.steps):
        ...
        if args.save_every > 0 and (step + 1) % args.save_every == 0:
            _save_checkpoints(
                ...
                step=step + 1,
                extra={"config_path": args.config, "config_hash": config_hash, "model_config": asdict(model_config)},
            )
    ...
    if args.output_dir is not None:
        output_dir = Path(args.output_dir)
        write_json(output_dir / "pixel_summary.json", summary)
        ...
        write_run_md(
            output_dir / "RUN.md",
            {
                ...
                "paths": {
                    "pixel_summary_json": str(output_dir / "pixel_summary.json"),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage-7 pixel-space grouped smoke trainer")
    ...
    parser.add_argument("--negatives-per-group", type=int, default=2)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to MAE encoder export payload from scripts/train_mae.py",
    )
    parser.add_argument(
        "--mae-encoder-arch",
    ...
    parser.add_argument("--queue-prime-samples", type=int, default=200)
    parser.add_argument("--queue-push-batch", type=int, default=32)
    parser.add_argument("--queue-per-class-capacity", type=int, default=64)
    parser.add_argument("--queue-global-capacity", type=int, default=400)
...
def _build_feature_extractor(*, args: argparse.Namespace, device: torch.device) -> nn.Module:
...
def _build_convnext_feature_extractor(*, args: argparse.Namespace, device: torch.device) -> nn.Module:
    from torchvision.models import ConvNeXt_Tiny_Weights, convnext_tiny

    if int(args.channels) != 3:
        raise ValueError("convnext_tiny feature encoder requires --channels 3")
    weights = None
...
def _maybe_compile_generator_forward(
    *,
    ...
    compiled_forward, compile_result = maybe_compile_callable(
        ...
        device=model_device,
        context="train_pixel.compile_generator",
    )
...
def _prime_queue(
    *,
    ...
    target_labels = torch.arange(target_classes, device=device, dtype=torch.long)
    attempts = 0
    max_attempts = max(8, target_classes * 4)
    while attempts < max_attempts:
        counts = torch.bincount(labels, minlength=num_classes)[:target_classes]
        if torch.all(counts >= warmup_min_per_class):
            break
        extra_images, extra_labels = _sample_real_batch(provider=provider, count=sample_count, device=device)
        images = torch.cat([images, extra_images], dim=0)
        labels = torch.cat([labels, extra_labels], dim=0)
        attempts += 1
    counts = torch.bincount(labels, minlength=num_classes)[:target_classes]
    if not torch.all(counts >= warmup_min_per_class):
        missing_labels = [int(index) for index, count in enumerate(counts.tolist()) if count < warmup_min_per_class]
        raise RuntimeError(
    ...
    for label in target_labels.tolist():
        label_matches = torch.where(labels == label)[0]
        selected_masks.append(label_matches[:warmup_min_per_class])
    selected_indices = torch.cat(selected_masks, dim=0)
    selected_mask = torch.zeros(labels.shape[0], dtype=torch.bool, device=device)
    selected_mask[selected_indices] = True
    remaining_capacity = sample_count - selected_indices.shape[0]
    if remaining_capacity > 0:
        remaining_indices = torch.where(~selected_mask)[0][:remaining_capacity]
        selected_indices = torch.cat([selected_indices, remaining_indices], dim=0)
    queue.push(images[selected_indices], labels[selected_indices])

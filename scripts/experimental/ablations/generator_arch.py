...
def _build_variants() -> list[dict[str, object]]:
    return [
        {
            "name": "baseline_table8_like_auto",
            "description": "Legacy auto wiring: RMSNorm + use_qk_norm + use_rope + register=16 + style(64x32).",
            "overrides": {},
        },
        {
            "name": "explicit_legacy_modes",
            "description": "Explicitly pin equivalent legacy modes: qk=l2, rope=1d_flat.",
            "overrides": {
                "qk_norm_mode": "l2",
                "rope_mode": "1d_flat",
            },
        ...
        {
            "name": "rope2d_axial",
            "description": "Use 2D axial RoPE for patch grid tokens.",
            "overrides": {
                "qk_norm_mode": "l2",
                "rope_mode": "2d_axial",
            },
        ...
        {
            "name": "rope2d_no_abs_pos",
            "description": "2D axial RoPE with patch absolute positional embedding disabled.",
            "overrides": {
                "qk_norm_mode": "l2",
                "rope_mode": "2d_axial",
        ...
        {
            "name": "rope2d_no_abs_pos_rms_noaffine",
            "description": "2D axial RoPE, no patch abs-pos, RMSNorm without affine gain.",
            "overrides": {
                "qk_norm_mode": "l2",
                "rope_mode": "2d_axial",
                "use_patch_positional_embedding": False,
        ...
        {
            "name": "fourier_alpha_rope2d_no_abs_pos",
            "description": "Fourier alpha embedding + 2D axial RoPE + no patch abs-pos.",
            "overrides": {
...
def _base_hparams(*, device: str, steps: int, seed: int) -> dict[str, object]:
    ...
    return {
        ...
        "qk_norm_mode": "auto",
        "rope_mode": "auto",
        "use_patch_positional_embedding": True,
        "rmsnorm_affine": True,
        "temperature": 0.05,
        "learning_rate": 1e-4,
...
def _build_command(*, config: dict[str, object], output_dir: Path) -> list[str]:
    ...
    mapping: tuple[tuple[str, str], ...] = (
        ...
        ("norm_type", "--norm-type"),
        ("alpha_embedding_type", "--alpha-embedding-type"),
        ("qk_norm_mode", "--qk-norm-mode"),
        ("rope_mode", "--rope-mode"),
    ...
    if not bool(config.get("use_patch_positional_embedding", True)):
        command.append("--disable-patch-positional-embedding")
    if not bool(config.get("rmsnorm_affine", True)):
        command.append("--disable-rmsnorm-affine")
